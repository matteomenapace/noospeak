...
function get_sentence ($rankedArray, $rawArray) {
	...
	return array($sentence,$index);
	
}

/*function get_url ($rankedArray, $rawArray, $htmlString, $needles){
	$index = $rankedArray [0]["reference"]["pageRank"];
	$description = $rawArray[$index];
	
	$descriptionPos = strpos($htmlString,$description);
	
	$cropHtml = trim(substr($htmlString,0,$descriptionPos));
	
	$start = strrpos($cropHtml,$needles["start"]);
	$end = strrpos($cropHtml,$needles["end"]);
	$url = trim(substr($cropHtml,$start,$end));
	
	echo ("|| description ->".$description);
	echo ("|| descriptionPos ->".$descriptionPos);
	echo ("|| needles start ->".$needles["start"]." end ->".$needles["end"]);
	echo ("|| start ->".$start);
	echo ("|| end ->".$end);
	echo ("|| url ->".$url);
