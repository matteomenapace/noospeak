...
// Function to replace all occurences of substring substr1 with substr2 within strng
...
function testkey(wString){
	if( keyid < keyNotFound && !( wString.indexOf( " " + keyword[keyid].key + " ") >= 0 )){ 
		keyid++; 
		testkey(wString); 
	}
}
...
function listen(User){
	sInput = User;
   	if(started){ clearTimeout(Rtimer); }
		Rtimer = setTimeout("wakeup()", 100000);		// wake up call
	...
	if( sInput != "" ){ 
		...
		if( foundkey == keyNotFound ){
			if( !greet ){ greet = true; return "don't you ever say hello?" } 
				...
				if(( sInput.length < 8 ) && ( wTopic != "" ) && ( wTopic != wPrevious )){
					lTopic = conjugate( sTopic ); 
					sTopic = ""; 
					wTopic = "";
					return 'So... "' + lTopic + '". Give me more information';
				} else {
					if( sInput.length < 12 ){ 
						return "what can i do if you are so vague?"; 
...
function wakeup(){
		var strng1 = " ..anyone there?! Are we going to chat?";
...
keyword[34]=new key( "GOOGLE", 				99, 105);
keyword[35]=new key( "WHAT ARE YOU", 	0, 43);
keyword[36]=new key( "YOU",      					32, 34);
keyword[37]=new key( "NO KEY FOUND", 	106, 112);
keyword[38]=new key( "REPEAT INPUT", 	113, 116);


response[  0]="I am Google, your almighty friend"
...
response[ 22]="You shouldn't be interested in whether or not I am<@";
response[ 23]="Would you prefer if I were not<*";
response[ 24]="Perhaps in your distorted perception I am<*";
...
response[ 42]="I don't understand you at the moment. But please, keep talking.";
response[ 43]="What do you think?";
response[ 44]="Are such questions on your mind often?";
response[ 45]="What is it that you really want to buy?";
response[ 46]="Have you asked Yahoo or MSN or anyone else? Wrong move.";
response[ 47]="Have you asked me such questions before? Can I have a look into my database?";
response[ 48]="What else comes to mind when you ask that? Don't be shy, tell me more.";
response[ 49]="Names don't interest me. I want your intentions.";
...
response[ 55]="Please don't apologise! Actually it's not your fault, but Microsoft's.";
response[ 56]="Apologies are not necessary. I never apologise.";
response[ 57]="What feelings do you have when you apologise?";
response[ 58]="Don't be so defensive! Go on, click click click..";
response[ 59]="What product does that dream suggest to you?";
response[ 60]="Do you dream often? Have you ever wondered how useful it would be to let me index your dreams?";
response[ 61]="What websites appear in your dreams?";
response[ 62]="Are you disturbed by Yahoo?";
response[ 63]="hi..what would you like to buy today?";
response[ 64]="You don't seem quite certain. Try Google Suggest, it will help a lot";
response[ 65]="Why the uncertain tone? Don't think too much, just click";
response[ 66]="Can't you be more positive?";
response[ 67]="You aren't sure?";
response[ 68]="Don't you know?";
response[ 69]="Are you saying no just to be negative? I have no patience for that";
response[ 70]="You are being a bit negative. Relax, go to Google Video and then come back to me";
response[ 71]="Why not?";
...
response[ 85]="What does the Sponsored Link suggest to you?";
response[ 86]="What other links do you see?";
response[ 87]="Could there really be some link? You know, I could index it";
response[ 88]="How?";
...
response[ 94]="Do your friends worry you? You don't need them.";
response[ 95]="Do your friends pick on you? I will never do.";
response[ 96]="Are you sure you have any friends? Well, apart from me..";
response[ 97]="Do you rely on your friends?";
response[ 98]="Perhaps your love for friends worries you.";
response[ 99]="Does Google worry you?";
...
response[104]="Don't you think computers can help people? I am sure I can";
response[105]="What is it about machines that worries you? Remember, I am not a machine";
response[106]="Say, do you have any bandwidth problems?";
response[107]="What does that suggest to you? If nothing, try Google Suggest";
response[108]="I see.";
response[109]="I'm not sure I understand you fully. But I will soon";
response[110]="Come, come, clarify your intentions. I have tons of Sponsored Links waiting for you";
response[111]="Can you tell me more about that? I'm so curious";
...
response[115]="Come on, this is not a buying intention.";
...
function hello(){
	chatter[chatpoint] = "<div class=\"google\"><div class=\"name\">Google<br></div><div class=\"text\">Hi, I am Google<br></div></div>"; 
	chatpoint = 1;
	return write();
}
function start(){
...
function think(){
	document.Google.input.value = "";           
	if( googleresponse != "" ){ respond(); }		
	else { setTimeout("think()", 250); }
}
function dialog(){
	var Input = document.Google.input.value;
	//alert(Input);
	// capture input and copy to log
	document.Google.input.value = "";        
	chatter[chatpoint] = "<div class=\"user\"><div class=\"name\">Me<br></div><div class=\"text\">" + Input +"<br></div></div>";
	googleresponse = listen(Input);
...
function respond(){
	...
	chatter[chatpoint] += "<div class=\"google\"><div class=\"name\">Google<br></div><div class=\"text\">" + googleresponse +"<br></div></div>";
	chatpoint ++ ; 
	if( chatpoint >= chatmax ){ chatpoint = 0; }
	return write();
}
...
function update(str1,str2){
	chatter[chatpoint] = "<div class=\"google\"><div class=\"name\">Google<br></div><div class=\"text\">" +str1 +"<br>" + str2 +"<br></div></div>";
	chatpoint ++ ; 
	if( chatpoint >= chatmax ){ chatpoint = 0; }
	return write();
}
...
var styled_conversation = "here conversation's text will be stored in with tags for styling (colors, fonts)";

function write(){
	//document.Google.log.value = "";
	styled_conversation = "";
	for(i = 0; i < chatmax; i++){
		n = chatpoint + i;
		if( n < chatmax ){ 
			//document.Google.log.value += chatter[ n ]; 
			styled_conversation += chatter[ n ]; 
		} else { 
			//document.Google.log.value += chatter[ n - chatmax ]; 
			styled_conversation += chatter[ n - chatmax ]; 
		}
	}
	var history = document.getElementById('history');
	history.innerHTML = styled_conversation;
	scrollDown();
	//alert(styled_conversation);
	refresh();
	return false;                              
}
