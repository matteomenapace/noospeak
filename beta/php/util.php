...
function g_logconversation($input,$response){
	global $selectbot;
	$uid = session_id();
	$input=addslashes($input);
	$response=strip_tags($response);
	$response=addslashes($response);
	$query="insert into g_conversationlog (uid,input,response,bot) values ('$uid','$input','$response','$selectbot')";
	$selectcode = mysql_query($query);
	}

/**
*get the conversation user-bot so far and style it
*/
function getconversation(){
	$conversation = array();
	global $uid;
	//echo ($uid);
	$query="select * from conversationlog where uid='$uid' order by id";
	$result = mysql_query($query);
	
	if ($result){
		//echo ("there is a result mate!");
		if (!mysql_numrows($result)) {
			return "";
		}else{
			while ($row = mysql_fetch_array($result)){
				$styledArray=array();
				$styledArray=style_conversation($row);
				$conversation[] = $styledArray['input'];
				$conversation[] = $styledArray['response'];	
			}
			return $conversation;
			//echo ($conversation);
		}
...
/**
*get the conversation user-google so far and style it
*/
function g_getconversation(){
	$conversation = array();
	global $uid;
	//echo ($uid);
	$query="select * from g_conversationlog where uid='$uid' order by id";
	$result = mysql_query($query);
...
function style_conversation($row) {

	$input = $row['input'];
	$response = $row['response'];
	$time = $row['enteredtime'];
	
	// int mktime ( int hour, int minute, int second, int month, int day, int year [, int is_dst] )
	// string from mySql -> 2007-03-05 18:57:05 
	$timestamp=mktime(substr($time,11,2), substr($time,14,2), 0, substr($time,5,2), substr($time,8,2), substr($time,0,4));
	$sTime= date("H:i D d.m.Y", $timestamp);
	
	$sInput = "<div class='user'>
						<div class='name'>Me</div>
						<div class='text'>" . $input . "</div>
						<div class='info'>" . $sTime . " somewhere - <a href=#>similar topic</a></div>
					</div>";
	$sRresponse = "<div class='google'>
